...
echo 1. 首先获取dev命名空间ID：
for /f "tokens=2 delims=:" %%i in ('curl -s "http://localhost:8848/nacos/v1/console/namespaces" ^| findstr "dev"') do (
...
if "%namespaceId%"=="" (
    echo 警告：无法获取dev命名空间ID，可能服务在public命名空间
    set "namespaceId=public"
)

echo 命名空间ID: %namespaceId%
echo.

echo 2. 获取dev命名空间服务列表：
curl -s "http://localhost:8848/nacos/v1/ns/service/list?pageNo=1&pageSize=10&namespaceId=%namespaceId%"
echo.

echo 3. 检查各服务实例（使用dev命名空间）：
for %%s in (catalog-service user-service enrollment-service) do (
    echo 服务 %%s 实例：
    curl -s "http://localhost:8848/nacos/v1/ns/instance/list?serviceName=%%s&namespaceId=%namespaceId%"
    echo.
)

echo 请现在打开Nacos控制台截图服务列表！
echo 注意：登录后切换到dev命名空间查看
echo 按任意键继续（截图完成后）...
pause > nul
...
echo   注意：服务在dev命名空间，登录后请切换到dev命名空间
echo.
echo ✓ 需要截图的内容：
echo   1. Nacos服务列表（dev命名空间）
